...
Application.Quit ();
    ...
    public void ProgressBarSetValue(float value)
    {
        ...
        _progressBar.GetText.text = String.Format("{0}%",
        Math.Truncate(value * 100));
    }
    public void ProgressBarEnabled()
    {
        if (_progressBar) return;
        _progressBar = Instantiate(InterfaceResources.ProgressbarPrefab,
        InterfaceResources.MainCanvas.transform);
    ...
    private IEnumerator LoadSceneAsync(AsyncOperation async)
    {
        ...
        while (!async.isDone)
        {
            ...
            if (async.progress < 0.9f && Mathf.RoundToInt(progress) !=
            100)
            {
                async.allowSceneActivation = false;
            }
